...
class LeaderboardControllerTest {
    ...
    void getLeaderboard_WhenTokenValid_ReturnsOk() throws Exception {
        ...
        when(leaderboardService.getLeaderboard(USER_ID, 10)).thenReturn(response);
    ...
    void getLeaderboard_WhenCustomLimitProvided_PassesLimitToService() throws Exception {
        LeaderboardResponse response = new LeaderboardResponse("Hillcrest", "averageRating", List.of(), null);

        when(firebaseAuthService.getUserIdFromToken(RAW)).thenReturn(USER_ID);
        when(leaderboardService.getLeaderboard(USER_ID, 5)).thenReturn(response);
